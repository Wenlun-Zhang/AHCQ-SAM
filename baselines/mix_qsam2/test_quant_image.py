...
MIX_QSAM2_ROOT = Path(__file__).resolve().parent
BASELINES_ROOT = MIX_QSAM2_ROOT.parent
REPO_ROOT = BASELINES_ROOT.parent
ROOT = REPO_ROOT / "sam2"
SAM2_REPO = ROOT / "sam2"
AHCQSAM_ROOT = ROOT / "ahcqsam"
SAM2_IMAGE_ROOT = BASELINES_ROOT / "sam2_image"
for path in (BASELINES_ROOT, ROOT, SAM2_REPO, AHCQSAM_ROOT, SAM2_IMAGE_ROOT, MIX_QSAM2_ROOT):
    path_str = str(path)
...
from recon import recon_model_image  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate SAM2 image segmentation on COCO GT box prompts.")
    ...
    parser.add_argument("--q_config", type=str, default="./mix_qsam2/exp_image/config44.yaml")
...
def normalize_quant_config(q_config):
    ...
    q_config.wrapper_compat = compat_config
    q_config.ahcqsam = compat_config
    return q_config
...
def quantize_model(model: torch.nn.Module, config_quant) -> torch.nn.Module:
    ...
    def replace_module(module, w_qconfig, a_qconfig, compat_config, ptq4sam_config):
...
def select_calibration_records(
    ...
    return list(dataset.iter_image_ids(image_ids))


@torch.inference_mode()
def forward_record_distribution(
    predictor: SAM2ImagePredictor,
    record: CocoImageRecord,
    eps: float,
    multimask_output: bool,
    ...
    for instance in record.instances:
        ...
        if isinstance(masks, np.ndarray) and masks.size > 0:
            values.append(torch.sigmoid(torch.from_numpy(masks).float()).flatten())
...
def main() -> None:
    ...
    if not args.fp_model:
        ...
        if mix_cfg is not None and mix_cfg.enabled:
            score_records = select_calibration_records(
                calib_dataset,
                int(mix_cfg.score_samples),
                args.calib_seed,
            )
            bit_allocation = search_mixed_precision_bits(
                model=predictor.model,
                train_dataset=score_records,
                q_config=q_config,
                forward_distribution=lambda _model, record, eps, max_frames, sis_config: (
                    forward_record_distribution(
                        predictor,
                        record,
                        eps,
            ...
            apply_mixed_precision_bits(predictor.model, bit_allocation)
        ...
        needs_fp_counterpart = args.recon
        fp_predictor = None
    ...
    summary["mix_qsam2_allocation"] = bit_allocation is not None
    summary["recon"] = bool(args.recon) and not bool(args.fp_model)
