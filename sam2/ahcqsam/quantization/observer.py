...
from .util_quant import fake_quantize_per_tensor_affine, fake_quantize_per_channel_affine, fake_logquantize_per_tensor_affine, fake_hybrid_quantize_per_tensor_affine, fake_log_transform_quantize
...
class LogTransformObserver(AvgMSEFastObserver):

    def __init__(self, bit=8, symmetric=False, ch_axis=-1):
        super().__init__(bit, symmetric, ch_axis)

        self.register_buffer("best_alpha_scales", torch.tensor(float("inf")))
        self.register_buffer("alpha_errors", torch.tensor(float("inf")))

        self.alphas = [0.001, 0.1, 1, 5, 10.0, 20.0, 50.0, 100.0, 200.0]

        self.value = None

    def loss_fx(self, x, new_max, alpha):
        scale = torch.tensor(new_max, device=x.device, dtype=x.dtype)

        x_q = fake_log_transform_quantize(
            x, scale, alpha, self.quant_min, self.quant_max
        )
    ...
    def forward(self, x_orig, value=None):
        ...
        x = x_orig.clone().detach().to(torch.float32)

        if self.one_side_dist is None:
            self.one_side_dist = 'pos' if x.min() >= 0.0 else 'neg' if x.max() <= 0.0 else 'no'

        if self.one_side_dist != 'no' or self.symmetric:
            batch_scales, batch_errors = self.golden_section_1D_search(x)

        if self.best_alpha_scales.ndim == 0 or self.best_alpha_scales.isinf().any():
            self.best_alpha_scales = batch_scales
            self.alpha_errors = batch_errors
        else:
            self.best_alpha_scales = self.best_alpha_scales * self.cnt + batch_scales
            self.alpha_errors = self.alpha_errors + batch_errors

        self.cnt += 1
        self.best_alpha_scales /= self.cnt
    ...
    def golden_section_1D_search(self, x):
        ...
        best_scales_list, errors_list = self.search_best_scale_for_each_alpha(x, x_max)

        return best_scales_list, errors_list

    def search_best_scale_for_each_alpha(self, x, x_max):
        xrange = x_max.abs().item()
        ...
        for alpha in self.alphas:
            result = minimize_scalar(
                self.golden_loss_wrapper,
                args=(x, alpha),
                bounds=(max(1e-4, 0.1 * xrange), xrange),
                method='Bounded',
            )

            best_scales.append(torch.tensor(result.x, device=x.device))
            errors.append(torch.tensor(result.fun, device=x.device))

        return torch.stack(best_scales), torch.stack(errors)

    def golden_loss_wrapper(self, search_scale, x, alpha):
        return self.loss_fx(x, search_scale, alpha)
