...
from .util_quant import (
    ...
    fake_logquantize_per_tensor_affine
)
...
class LogTransformQuantize(QuantizeBase):
    ...
    def __init__(self, observer, bit=8, symmetric=False, ch_axis=-1):
        ...
        self.alpha = torch.nn.Parameter(torch.tensor([100.0], dtype=torch.float))
    ...
    def ori_forward(self, x: torch.Tensor):
        if self.inited is False:
            best_scale, best_alpha = self.init_quantization_params(x)
            self.scale.data = best_scale
            self.alpha.data.fill_(best_alpha)
            self.inited = True

        x_dequant = self.quantize(x, self.scale, self.alpha)
        return x_dequant
    ...
    def forward(self, X, value=None):
        ...
        if self.fake_quant_enabled == 1:
            if hasattr(self, 'drop_prob') and self.drop_prob < 1.0:
                x_orig = X
    ...
    def init_quantization_params(self, x: torch.Tensor):
        alpha_errors = self.observer.alpha_errors
        _, min_error_idx = torch.min(alpha_errors, dim=0)

        best_scale = self.observer.best_alpha_scales[min_error_idx]
        best_alpha = self.observer.alphas[min_error_idx]
        return best_scale, best_alpha
...
class HybridQuantize(QuantizeBase):
    ...
    def init_quantization(self):
        ...
        scale_uni = range * (1 - best_range_rate) / (
                    (self.observer.quant_max - self.observer.quant_min) * (1 - best_grid_rate))
