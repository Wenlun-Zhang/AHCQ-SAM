...
from ahcqsam.quantization.quantized_module import QuantizedLayer, QuantizedBlock, Quantizer  # noqa: F401
from projects.instance_segment_anything.models.segment_anything.modeling.transformer import Attention, \
    TwoWayAttentionBlock
from projects.instance_segment_anything.models.segment_anything.modeling.common import MLPBlock
from ahcqsam.quantization.quantized_module import PreQuantizedLayer, QuantizedMatMul
...
from projects.instance_segment_anything.models.segment_anything.modeling.transformer import \
...
from projects.instance_segment_anything.models.segment_anything.modeling.image_encoder import window_partition, \
    window_unpartition, add_decomposed_rel_pos
...
class QuantEncoderMLPBlock(QuantizedBlock):
    def __init__(self, org_module: MLPBlock, w_qconfig, a_qconfig, ahcqsam_config, qinput=True):
        super().__init__()
...
class QuantDecoderOurTwoWayAttentionBlock(nn.Module):
    ...
    def __init__(self, org_module: TwoWayAttentionBlock, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config,
                 qoutput=True) -> None:
        super().__init__()
        self.self_attn = QuantDecoderOurAttentionBlock(org_module.self_attn, w_qconfig, a_qconfig, ahcqsam_config,
                                                       ptq4sam_config, qinput=True)
        self.norm1 = org_module.norm1

        self.cross_attn_token_to_image = QuantDecoderOurAttentionBlock(
            org_module.cross_attn_token_to_image, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config, qinput=True
        )
        self.norm2 = org_module.norm2

        self.mlp = QuantDecoderMLPBlock(org_module.mlp, w_qconfig, a_qconfig, ahcqsam_config, qinput=True)
        self.norm3 = org_module.norm3

        self.norm4 = org_module.norm4
        self.cross_attn_image_to_token = QuantDecoderOurAttentionBlock(
            org_module.cross_attn_image_to_token, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config, qinput=True
        )
...
class QuantDecoderOurAttentionBlock(QuantizedBlock):
    def __init__(self, org_module: DecoderAttention, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config, qoutput=True,
                 qinput=False):
        super().__init__()
        self.qoutput = qoutput
...
class QuantImageEncoderOurViT(nn.Module):
    ...
    def __init__(self, org_module: ImageEncoderViT, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config,
                 qoutput=True) -> None:
        super().__init__()
        self.img_size = org_module.img_size
        self.patch_embed = org_module.patch_embed
        ...
        for i in range(len(org_module.blocks)):
            self.blocks.append(
                QunatEncoderOurBlock(org_module.blocks[i], w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config))
...
class QunatEncoderOurBlock(nn.Module):
    def __init__(self, org_module: Block, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config, qoutput=True) -> None:
        super().__init__()
        self.norm1 = org_module.norm1
        self.attn = QuantEncoderOurAttentionBlock(org_module.attn, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config)
        # print(self.attn)
        self.norm2 = org_module.norm2
        self.mlp = QuantEncoderMLPBlock(org_module.mlp, w_qconfig, a_qconfig, ahcqsam_config)
...
class QuantEncoderOurAttentionBlock(QuantizedBlock):
    def __init__(self, org_module: EncoderAttention, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config, qoutput=False,
                 qinput=True):
        super().__init__()
        self.qinput = qinput
        self.qoutput = qoutput
