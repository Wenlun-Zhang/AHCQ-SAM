...
sys.path.append("/home/zhang/Project/AHCQ-SAM/sam")
import copy
import logging
import torch.nn as nn
import torch.distributed as dist
from ahcqsam.model.quant_model import specials
from ahcqsam.model.quant_model import bimodal_adjust
from ahcqsam.quantization.state import enable_calibration_woquantization, enable_quantization, disable_all
from ahcqsam.quantization.quantized_module import QuantizedLayer, QuantizedBlock, PreQuantizedLayer, QuantizedMatMul
from ahcqsam.model.quant_model import QunatEncoderOurBlock, QuantDecoderOurTwoWayAttentionBlock
from ahcqsam.quantization.fake_quant import QuantizeBase
from ahcqsam.quantization.observer import ObserverBase, AvgMSEObserver, AvgMinMaxObserver
from recon import reconstruction, condiBasedAct_reconstruction
import utils
...
def main():
    ...
    if args.work_dir is not None and rank == 0:
        ...
        logger = get_logger(name='ahcqsam', log_file=log_file, log_level=logging.INFO)
    ...
    if not args.fp:
        if len(args.load_sam_path) == 0:
            ...
            logger.info('acnr')
            logger.info(q_config.ahcqsam.acnr)
            logger.info('cag')
            logger.info(q_config.ahcqsam.cag)
            logger.info('hluq')
            logger.info(q_config.ahcqsam.hluq)
            logger.info('lnq')
            logger.info(q_config.ahcqsam.lnq)
            logger.info('do sign')
            ...
            if q_config.ahcqsam.acnr:
                calibrate(model, cali_data, q_config.ptq4sam.BIG, only_act=True)
                print('ACNR Reconstruction Start...')
                condiBasedAct_recon_model(model, fp_model, cali_data, q_config.recon, q_config.ahcqsam, brecq=brecq)
                calibrate(model, cali_data, False, only_act=False)
            else:
                calibrate(model, cali_data, q_config.ptq4sam.BIG, only_act=False)

            if hasattr(q_config, 'recon'):
                if rank == 0:
                    logger.info('begin to do reconstruction')
                recon_model(model, fp_model, cali_data, q_config.recon, q_config.ahcqsam, brecq=brecq)
...
def quantize_model(model, config_quant, args):
    def replace_module(module, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config, qoutput=True):
        for name, child_module in module.named_children():
            ...
            if type(child_module) in specials:
                setattr(module, name,
                        specials[type(child_module)](child_module, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config))
                ...
                replace_module(child_module, w_qconfig, a_qconfig, ahcqsam_config, ptq4sam_config, qoutput)

    if args.quant_encoder:
        model.predictor.model.image_encoder = specials[type(model.predictor.model.image_encoder)](
            model.predictor.model.image_encoder, config_quant.w_qconfig, config_quant.a_qconfig, config_quant.ahcqsam,
            config_quant.ptq4sam)

    replace_module(model.predictor.model.mask_decoder, config_quant.w_qconfig, config_quant.a_qconfig,
                   config_quant.ahcqsam, config_quant.ptq4sam)
...
@torch.no_grad()
def calibrate(model, cali_data, BIG, only_act=True):
    st = time.time()
...
def condiBasedAct_recon_model(model, fp_model, cali_data, recon_config, ahcqsam_config, brecq=False):
    if len(cali_data) > 16:
    ...
    def _condiBasedAct_recon_model(module, fp_module, prefix=""):
        for name, child_module in module.named_children():
            ...
            if isinstance(child_module, (QuantizedLayer, PreQuantizedLayer)):
                logger.info('begin condiBasedAct reconstruction for module [{}]:\n'.
                            format(global_name))

                cond_num_before_optimization, cond_num_after_optimization = \
                    condiBasedAct_reconstruction(model, fp_model, child_module,
                                                 getattr(fp_module, name), cali_data, recon_config, ahcqsam_config)

                with open('log_cond_num.txt', 'a') as f:
                    f.write('module [{}], cond_num_before_optimization: {}, ' \
...
def recon_model(model, fp_model, cali_data, recon_config, ahcqsam_config, brecq=False):
    if len(cali_data) > 16:
        recon_config.keep_gpu = False
    ...
    def _recon_model(module, fp_module, prefix=""):
        for name, child_module in module.named_children():
            global_name = f"{prefix}.{name}" if prefix else name

            if isinstance(child_module, (QunatEncoderOurBlock, QuantDecoderOurTwoWayAttentionBlock,
                                         QuantizedBlock, QuantizedLayer, PreQuantizedLayer, QuantizedMatMul)):

                logger.info('begin reconstruction for module [{}]:\n{}'.format(global_name, str(child_module)))

                reconstruction(model, fp_model, child_module, getattr(fp_module, name), cali_data, recon_config,
                               ahcqsam_config)
            else:
                _recon_model(child_module, getattr(fp_module, name), global_name)

    _recon_model(model, fp_model, prefix="")
